...
extern DMA_QListTypeDef Dcmi_Queue;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
 volatile uint32_t usb_tx_complete = 0,FPS_640 = 0 ;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* Initialize all configured peripherals */
	...
	MX_Dcmi_Queue_Config();
  HAL_Delay(30);
	
	HAL_DMAEx_List_LinkQ(&handle_GPDMA1_Channel12, &Dcmi_Queue);
	HAL_Delay(30);
	
	__HAL_LINKDMA(&hdcmi, DMA_Handle, handle_GPDMA1_Channel12);
	HAL_Delay(30);
	
	HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_CONTINUOUS, (uint32_t)&Rx_B_Temp[0], 153600);
	HAL_Delay(30);
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  RCC_OscInitStruct.PLL.PLLN = 13;////9;
